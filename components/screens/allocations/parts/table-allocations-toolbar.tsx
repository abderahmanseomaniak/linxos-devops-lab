...
export function TableAllocationsToolbar({
  ...
  const hasFilters = !!search || (filters?.quantity_level?.length ?? 0) > 0
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex items-center gap-3">
        ...
        <StatusFilterPopover
          values={filters?.quantity_level}
          onChange={(v) => onFilterChange?.("quantity_level", v)}
        />
...
const QUANTITY_LEVEL_OPTIONS = [
  { value: "high", label: "Quantité élevée" },
  { value: "low", label: "Quantité faible" },
]

function StatusFilterPopover({ values = [], onChange }: { values?: string[]; onChange: (v: string[] | undefined) => void }) {
  const isAllChecked = values.length === 0 || values.length === QUANTITY_LEVEL_OPTIONS.length

  const handleAllChange = (checked: boolean) => {
    if (checked) {
      onChange(undefined)
    } else {
      onChange(QUANTITY_LEVEL_OPTIONS.map((o) => o.value))
    }
  }
  ...
  const handleOptionChange = (optionValue: string, checked: boolean) => {
    if (checked) {
      ...
      if (next.length === QUANTITY_LEVEL_OPTIONS.length) {
        onChange(undefined)
      } else {
        onChange(next)
      }
    ...
    <Popover>
      ...
      <PopoverContent align="start" className="w-auto min-w-44 p-3">
        <div className="space-y-3">
          <Typography variant="small" className="font-medium">Niveau de quantité</Typography>
          <div className="space-y-2">
            ...
            {QUANTITY_LEVEL_OPTIONS.map((opt) => (
              <div key={opt.value} className="flex items-center gap-2">
                ...
                <Label htmlFor={`alc-filter-${opt.value}`} className="cursor-pointer font-normal">{opt.label}</Label>
              </div>
            ))}
          </div>
        </div>
