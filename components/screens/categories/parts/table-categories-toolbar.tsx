...
import type { CategoryListFilters } from "../categories-table"

interface TableCategoriesToolbarProps {
  ...
  onFilterChange: <K extends keyof CategoryListFilters>(key: K, value: CategoryListFilters[K]) => void
}

export function TableCategoriesToolbar({
  table, onClearFilters,
  columnVisibility,
  filters, onFilterChange,
}: TableCategoriesToolbarProps) {
  const inputRef = useRef<HTMLInputElement>(null)

  const hasFilters = !!filters.search || (filters.is_active?.length ?? 0) > 0
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Checkbox checked={isAllChecked} id="cat-filter-all" onCheckedChange={handleAllChange} />
              <Label htmlFor="cat-filter-all" className="cursor-pointer font-normal">Tous</Label>
            </div>
            {STATUS_OPTIONS.map((opt) => (
              <div key={opt.value} className="flex items-center gap-2">
                <Checkbox checked={values.includes(opt.value)} id={`cat-filter-${opt.value}`} onCheckedChange={(checked) => handleOptionChange(opt.value, !!checked)} />
                <Label htmlFor={`cat-filter-${opt.value}`} className="cursor-pointer font-normal">{opt.label}</Label>
              </div>
            ))}
          </div>
        </div>
        ...
        {table.getAllColumns().filter((c) => c.getCanHide()).map((column) => (
          ...
          >
            {column.id === "name" ? "Nom" :
             ...
             column.id === "is_active" ? "Active" :
             column.id === "products_count" ? "Produits" : column.id}
          </DropdownMenuCheckboxItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
