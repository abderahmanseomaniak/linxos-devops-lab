...
import {
  IconAdjustments, IconCircleX, IconColumns3, IconFilter, IconPlus, IconRefresh,
...
export function ContentTableToolbar({
  table, onRefresh, search, onSearchChange, filterPlatform, onFilterChange, onAdd,
  columnVisibility,
}: ContentTableToolbarProps) {
  const inputRef = useRef<HTMLInputElement>(null)

  const hasFilters = !!search || (filterPlatform?.length ?? 0) > 0
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-3">
        {hasFilters && (
          <Button variant="ghost" className="h-8 text-xs" onClick={() => { onSearchChange(""); onFilterChange("platform", undefined) }}>
            Réinitialiser
          </Button>
        )}
        ...
        {onAdd && (
          <Button className="h-8 text-xs" onClick={onAdd}>
            <IconPlus className="size-3.5" /> Ajouter
          </Button>
        )}
      </div>
    </div>
...
function StatusFilterPopover({ values = [], onChange }: { values?: string[]; onChange: (v: string[] | undefined) => void }) {
  const isAllChecked = values.length === 0 || values.length === PLATFORM_OPTIONS.length

  const handleAllChange = (checked: boolean) => {
    if (checked) {
      onChange(undefined)
    } else {
      onChange(PLATFORM_OPTIONS.map((o) => o.value))
    }
  }
  ...
  const handleOptionChange = (optionValue: string, checked: boolean) => {
    if (checked) {
      ...
      if (next.length === PLATFORM_OPTIONS.length) {
        onChange(undefined)
      } else {
        onChange(next)
      }
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Checkbox checked={isAllChecked} id="cnt-filter-all" onCheckedChange={handleAllChange} />
              <Label htmlFor="cnt-filter-all" className="cursor-pointer font-normal">Tous</Label>
            </div>
            {PLATFORM_OPTIONS.map((opt) => (
              <div key={opt.value} className="flex items-center gap-2">
                <Checkbox checked={values.includes(opt.value)} id={`cnt-filter-${opt.value}`} onCheckedChange={(checked) => handleOptionChange(opt.value, !!checked)} />
                <Label htmlFor={`cnt-filter-${opt.value}`} className="cursor-pointer font-normal">{opt.label}</Label>
              </div>
            ))}
          </div>
        </div>
        ...
        {table.getAllColumns().filter((c) => c.getCanHide()).map((column) => (
          ...
          >
            {column.id === "event" ? "Événement" :
             ...
             column.id === "content_type" ? "Type" :
             column.id === "views" ? "Vues" :
             column.id === "likes" ? "Likes" :
             column.id === "comments" ? "Commentaires" :
             column.id === "url" ? "URL" :
