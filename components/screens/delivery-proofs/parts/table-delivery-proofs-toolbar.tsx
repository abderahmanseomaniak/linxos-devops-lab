...
interface DeliveryProofsTableToolbarProps {
  table: Table<DeliveryProof>
  onRefresh?: () => void
  search: string
  onSearchChange: (value: string) => void
  columnVisibility: Record<string, boolean>
  onColumnVisibilityChange: React.Dispatch<React.SetStateAction<Record<string, boolean>>>
  filters?: { has_description?: string[] }
  onFilterChange?: (key: string, value: string[] | undefined) => void
}

export function DeliveryProofsTableToolbar({
  table, onRefresh, search, onSearchChange,
  columnVisibility,
  filters, onFilterChange,
}: DeliveryProofsTableToolbarProps) {
  const inputRef = useRef<HTMLInputElement>(null)

  const hasFilters = !!search || (filters?.has_description?.length ?? 0) > 0
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex items-center gap-3">
        ...
        <StatusFilterPopover
          values={filters?.has_description}
          onChange={(v) => onFilterChange?.("has_description", v)}
        />
...
const DESCRIPTION_OPTIONS = [
  { value: "with_desc", label: "Avec description" },
  { value: "no_desc", label: "Sans description" },
]

function StatusFilterPopover({ values = [], onChange }: { values?: string[]; onChange: (v: string[] | undefined) => void }) {
  const isAllChecked = values.length === 0 || values.length === DESCRIPTION_OPTIONS.length

  const handleAllChange = (checked: boolean) => {
    if (checked) {
      onChange(undefined)
    } else {
      onChange(DESCRIPTION_OPTIONS.map((o) => o.value))
    }
  }
  ...
  const handleOptionChange = (optionValue: string, checked: boolean) => {
    if (checked) {
      ...
      if (next.length === DESCRIPTION_OPTIONS.length) {
        onChange(undefined)
      } else {
        onChange(next)
      }
    ...
    <Popover>
      ...
      <PopoverContent align="start" className="w-auto min-w-44 p-3">
        <div className="space-y-3">
          <Typography variant="small" className="font-medium">Description</Typography>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Checkbox checked={isAllChecked} id="dlv-filter-all" onCheckedChange={handleAllChange} />
              <Label htmlFor="dlv-filter-all" className="cursor-pointer font-normal">Tous</Label>
            </div>
            {DESCRIPTION_OPTIONS.map((opt) => (
              <div key={opt.value} className="flex items-center gap-2">
                <Checkbox checked={values.includes(opt.value)} id={`dlv-filter-${opt.value}`} onCheckedChange={(checked) => handleOptionChange(opt.value, !!checked)} />
                <Label htmlFor={`dlv-filter-${opt.value}`} className="cursor-pointer font-normal">{opt.label}</Label>
              </div>
            ))}
          </div>
        </div>
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        {table.getAllColumns().filter((c) => c.getCanHide()).map((column) => (
          ...
          >
            {column.id === "shipment_id" ? "Expédition" :
             column.id === "file_url" ? "Fichier" :
