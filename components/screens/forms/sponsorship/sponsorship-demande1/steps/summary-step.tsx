...
function SummarySection({ title, step, children }: { title: string; step: number; children: React.ReactNode }) {
  const edit = (onEdit as (step: number) => void)
  return (
    <Card>
...
function InternalSummaryStep({ onEdit: edit }: Props) {
  onEdit = edit
  ...
  return (
    <div className="space-y-3">
      ...
      <SummarySection title="Club / Association" step={1}>
      ...
      <SummarySection title="Partenariat & Événement" step={2}>
      ...
      <SummarySection title="Visibilité & contreparties" step={3}>
        <SummaryLine label="Visibilité proposée" value={values.visibiliteContreparties} />
      </SummarySection>

      <SummarySection title="UGC / Influenceurs" step={4}>
      ...
      <SummarySection title="Logistique, autorisations & fichiers" step={5}>
      ...
      <SummarySection title="Signature" step={7}>
