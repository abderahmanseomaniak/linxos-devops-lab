...
import {
  IconAdjustments, IconCheck, IconCircleX, IconColumns3, IconFilter, IconRefresh,
...
export function NotificationsTableToolbar({
  table, onRefresh, search, onSearchChange, filterIsRead, onFilterChange, onMarkAllAsRead, hasUnread,
  columnVisibility,
}: NotificationsTableToolbarProps) {
  const inputRef = useRef<HTMLInputElement>(null)

  const hasFilters = !!search || (filterIsRead?.length ?? 0) > 0
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex items-center gap-3">
        ...
        <StatusFilterPopover
          values={filterIsRead}
          onChange={(v) => onFilterChange("is_read", v)}
        />
      ...
      <div className="flex items-center gap-3">
        {hasFilters && (
          <Button variant="ghost" className="h-8 text-xs" onClick={() => { onSearchChange(""); onFilterChange("is_read", undefined) }}>
            Réinitialiser
          </Button>
        )}
        {hasUnread && onMarkAllAsRead && (
          <Button variant="outline" className="h-8 text-xs" onClick={onMarkAllAsRead}>
...
function StatusFilterPopover({ values = [], onChange }: { values?: string[]; onChange: (v: string[] | undefined) => void }) {
  const isAllChecked = values.length === 0 || values.length === READ_STATUS_OPTIONS.length

  const handleAllChange = (checked: boolean) => {
    if (checked) {
      onChange(undefined)
    } else {
      onChange(READ_STATUS_OPTIONS.map((o) => o.value))
    }
  }
  ...
  const handleOptionChange = (optionValue: string, checked: boolean) => {
    if (checked) {
      ...
      if (next.length === READ_STATUS_OPTIONS.length) {
        onChange(undefined)
      } else {
        onChange(next)
      }
    ...
    <Popover>
      ...
      <PopoverContent align="start" className="w-auto min-w-44 p-3">
        <div className="space-y-3">
          <Typography variant="small" className="font-medium">Statut lecture</Typography>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Checkbox checked={isAllChecked} id="ntf-filter-all" onCheckedChange={handleAllChange} />
              <Label htmlFor="ntf-filter-all" className="cursor-pointer font-normal">Tous</Label>
            </div>
            {READ_STATUS_OPTIONS.map((opt) => (
              <div key={opt.value} className="flex items-center gap-2">
                <Checkbox checked={values.includes(opt.value)} id={`ntf-filter-${opt.value}`} onCheckedChange={(checked) => handleOptionChange(opt.value, !!checked)} />
                <Label htmlFor={`ntf-filter-${opt.value}`} className="cursor-pointer font-normal">{opt.label}</Label>
              </div>
            ))}
          </div>
        </div>
        ...
        {table.getAllColumns().filter((c) => c.getCanHide()).map((column) => (
          ...
          >
            {column.id === "title" ? "Titre" :
             ...
             column.id === "notification_type" ? "Type" :
             column.id === "is_read" ? "Lu" :
