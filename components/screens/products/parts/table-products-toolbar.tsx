...
import type { ProductListFilters } from "../products-table"

interface TableProductsToolbarProps {
  table: Table<Product>
  onClearFilters?: () => void
...
export function TableProductsToolbar({
  table, onClearFilters,
  columnVisibility,
  filters, onFilterChange,
}: TableProductsToolbarProps) {
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Checkbox checked={isAllChecked} id="prd-filter-all" onCheckedChange={handleAllChange} />
              <Label htmlFor="prd-filter-all" className="cursor-pointer font-normal">Tous</Label>
            </div>
            {STATUS_OPTIONS.map((opt) => (
              <div key={opt.value} className="flex items-center gap-2">
                <Checkbox checked={values.includes(opt.value)} id={`prd-filter-${opt.value}`} onCheckedChange={(checked) => handleOptionChange(opt.value, !!checked)} />
                <Label htmlFor={`prd-filter-${opt.value}`} className="cursor-pointer font-normal">{opt.label}</Label>
              </div>
            ))}
          </div>
        </div>
