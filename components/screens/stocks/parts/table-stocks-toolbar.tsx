...
interface StocksTableToolbarProps {
  table: Table<CampaignStockView>
  onRefresh?: () => void
  search: string
...
export function StocksTableToolbar({
  ...
  columnVisibility,
}: StocksTableToolbarProps) {
  const inputRef = useRef<HTMLInputElement>(null)

  const hasFilters = !!search || (stockLevel?.length ?? 0) > 0
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex items-center gap-3">
        ...
        <StatusFilterPopover
          values={stockLevel}
          onChange={(v) => onStockLevelChange(v)}
        />
...
const STOCK_LEVEL_OPTIONS = [
  ...
  { value: "empty", label: "Rupture" },
]

function StatusFilterPopover({ values = [], onChange }: { values?: string[]; onChange: (v: string[] | undefined) => void }) {
  const isAllChecked = values.length === 0 || values.length === STOCK_LEVEL_OPTIONS.length

  const handleAllChange = (checked: boolean) => {
    if (checked) {
      onChange(undefined)
    } else {
      onChange(STOCK_LEVEL_OPTIONS.map((o) => o.value))
    }
  }
  ...
  const handleOptionChange = (optionValue: string, checked: boolean) => {
    if (checked) {
      ...
      if (next.length === STOCK_LEVEL_OPTIONS.length) {
        onChange(undefined)
      } else {
        onChange(next)
      }
    ...
    <Popover>
      ...
      <PopoverContent align="start" className="w-auto min-w-44 p-3">
        <div className="space-y-3">
          <Typography variant="small" className="font-medium">Statut stock</Typography>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Checkbox checked={isAllChecked} id="stk-filter-all" onCheckedChange={handleAllChange} />
              <Label htmlFor="stk-filter-all" className="cursor-pointer font-normal">Tous</Label>
            </div>
            {STOCK_LEVEL_OPTIONS.map((opt) => (
              <div key={opt.value} className="flex items-center gap-2">
                <Checkbox checked={values.includes(opt.value)} id={`stk-filter-${opt.value}`} onCheckedChange={(checked) => handleOptionChange(opt.value, !!checked)} />
                <Label htmlFor={`stk-filter-${opt.value}`} className="cursor-pointer font-normal">{opt.label}</Label>
              </div>
            ))}
          </div>
        </div>
        ...
        {table.getAllColumns().filter((c) => c.getCanHide()).map((column) => (
          ...
          >
            {column.id === "campaign_name" ? "Campagne" :
             ...
             column.id === "category_name" ? "Catégorie" :
             column.id === "total_quantity" ? "Total" :
             column.id === "available_quantity" ? "Disponible" :
             column.id === "reserved_quantity" ? "Réservé" :
             column.id === "consumed_quantity" ? "Consommé" : column.id}
          </DropdownMenuCheckboxItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
