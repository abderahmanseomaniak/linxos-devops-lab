...
import {
  IconAdjustments, IconCircleX, IconColumns3, IconFilter, IconRefresh,
...
interface TableVerificationsToolbarProps {
  table: Table<UGCContent>
  onRefresh?: () => void
  search: string
  onSearchChange: (value: string) => void
  columnVisibility: Record<string, boolean>
  onColumnVisibilityChange: React.Dispatch<React.SetStateAction<Record<string, boolean>>>
  filters?: { verification_status?: string[] }
  onFilterChange?: (key: string, value: string[] | undefined) => void
}

export function TableVerificationsToolbar({
  table, onRefresh, search, onSearchChange,
  columnVisibility,
  filters, onFilterChange,
}: TableVerificationsToolbarProps) {
  const inputRef = useRef<HTMLInputElement>(null)

  const hasFilters = !!search || (filters?.verification_status?.length ?? 0) > 0
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex items-center gap-3">
        ...
        <StatusFilterPopover
          values={filters?.verification_status}
      ...
      <div className="flex items-center gap-3">
        {hasFilters && (
          <Button variant="ghost" className="h-8 text-xs" onClick={() => onSearchChange("")}>
            Réinitialiser
          </Button>
        )}
...
function StatusFilterPopover({ values = [], onChange }: { values?: string[]; onChange: (v: string[] | undefined) => void }) {
  const isAllChecked = values.length === 0 || values.length === VERIFICATION_STATUS_OPTIONS.length

  const handleAllChange = (checked: boolean) => {
    if (checked) {
      onChange(undefined)
    } else {
      onChange(VERIFICATION_STATUS_OPTIONS.map((o) => o.value))
    }
  }
  ...
  const handleOptionChange = (optionValue: string, checked: boolean) => {
    if (checked) {
      ...
      if (next.length === VERIFICATION_STATUS_OPTIONS.length) {
        onChange(undefined)
      } else {
        onChange(next)
      }
    ...
    <Popover>
      ...
      <PopoverContent align="start" className="w-auto min-w-44 p-3">
        <div className="space-y-3">
          <Typography variant="small" className="font-medium">Statut vérification</Typography>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Checkbox checked={isAllChecked} id="vrf-filter-all" onCheckedChange={handleAllChange} />
              <Label htmlFor="vrf-filter-all" className="cursor-pointer font-normal">Tous</Label>
            </div>
            {VERIFICATION_STATUS_OPTIONS.map((opt) => (
              <div key={opt.value} className="flex items-center gap-2">
                <Checkbox checked={values.includes(opt.value)} id={`vrf-filter-${opt.value}`} onCheckedChange={(checked) => handleOptionChange(opt.value, !!checked)} />
                <Label htmlFor={`vrf-filter-${opt.value}`} className="cursor-pointer font-normal">{opt.label}</Label>
              </div>
            ))}
          </div>
        </div>
        ...
        {table.getAllColumns().filter((c) => c.getCanHide()).map((column) => (
          ...
          >
            {column.id === "event" ? "Événement" :
             column.id === "platform" ? "Plateforme" :
             column.id === "global_score" ? "Score global" :
             column.id === "visibility_score" ? "Score visibilité" :
             column.id === "quality_score" ? "Score qualité" :
             column.id === "engagement_score" ? "Score engagement" :
