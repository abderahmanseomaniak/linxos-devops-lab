...
interface WorkflowStatesTableToolbarProps {
  table: Table<WorkflowState>
  onRefresh?: () => void
  search: string
...
export function WorkflowStatesTableToolbar({
  table, onRefresh, search, onSearchChange, filterCode, onFilterChange, onAdd,
  columnVisibility,
}: WorkflowStatesTableToolbarProps) {
  const inputRef = useRef<HTMLInputElement>(null)

  const hasFilters = !!search || (filterCode?.length ?? 0) > 0
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-3">
        {hasFilters && (
          <Button variant="ghost" className="h-8 text-xs" onClick={() => { onSearchChange(""); onFilterChange("code", undefined) }}>
            Réinitialiser
          </Button>
        )}
    ...
    <Popover>
      ...
      <PopoverContent align="start" className="w-auto min-w-44 p-3">
        <div className="space-y-3">
          <Typography variant="small" className="font-medium">Code workflow</Typography>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Checkbox checked={isAllChecked} id="wfs-filter-all" onCheckedChange={handleAllChange} />
              <Label htmlFor="wfs-filter-all" className="cursor-pointer font-normal">Tous</Label>
            </div>
            {WORKFLOW_CODE_OPTIONS.map((opt) => (
              <div key={opt.value} className="flex items-center gap-2">
                <Checkbox checked={values.includes(opt.value)} id={`wfs-filter-${opt.value}`} onCheckedChange={(checked) => handleOptionChange(opt.value, !!checked)} />
                <Label htmlFor={`wfs-filter-${opt.value}`} className="cursor-pointer font-normal">{opt.label}</Label>
              </div>
            ))}
          </div>
        </div>
